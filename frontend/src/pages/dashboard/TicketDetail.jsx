...
import { LayoutDashboard, Ticket, Users, LogOut, ArrowLeft, Sparkles, FileText, Menu } from 'lucide-react'
      ...
      <main style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{
          ...
          <button onClick={handleSummarize} disabled={summarizing} style={{
            ...
            border: '1px solid #2A2A2A', color: summarizing ? '#444' : '#888',
            padding: '7px 12px', fontSize: '11px', fontFamily: 'JetBrains Mono, monospace',
            cursor: summarizing ? 'not-allowed' : 'pointer', letterSpacing: '0.05em'
          }}>
            <FileText size={11} />
        ...
        <div style={{ padding: '20px 16px', flex: 1 }}>
          <Link to="/tickets" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', color: '#555', textDecoration: 'none', fontSize: '12px', marginBottom: '20px', fontFamily: 'JetBrains Mono, monospace' }}>
            <ArrowLeft size={12} /> BACK
          </Link>

          {/* Ticket header */}
          <div style={{ border: '1px solid #1F1F1F', borderLeft: `3px solid ${statusColor(ticket?.status)}`, background: '#0D0D0D', padding: '20px', marginBottom: '16px' }}>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#444', marginBottom: '6px' }}>#{ticket?.id}</div>
            <h1 style={{ fontSize: '18px', fontWeight: 700, margin: '0 0 12px', letterSpacing: '-0.02em' }}>{ticket?.title}</h1>
            ...
            <p style={{ fontSize: '14px', color: '#999', lineHeight: 1.6, margin: '0 0 10px' }}>{ticket?.body}</p>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: '#444' }}>BY {ticket?.user?.name?.toUpperCase()}</div>

            {summary && (
              <div style={{ marginTop: '16px', padding: '14px', background: '#111', borderLeft: '2px solid #F5A623' }}>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: '#F5A623', letterSpacing: '0.1em', marginBottom: '6px' }}>AI SUMMARY</div>
          ...
          {/* Messages */}
          <div style={{ border: '1px solid #1F1F1F', background: '#0D0D0D', marginBottom: '16px' }}>
            ...
            {messages.length === 0 ? (
              <div style={{ padding: '32px', textAlign: 'center', color: '#444', fontFamily: 'JetBrains Mono, monospace', fontSize: '11px' }}>NO MESSAGES YET</div>
            ) : (
              <div style={{ padding: '12px 16px' }}>
                {messages.map((msg, i) => (
                  <div key={msg.id} style={{
                    padding: '14px', marginBottom: i < messages.length - 1 ? '10px' : 0,
                    background: msg.isAI ? '#0F0F18' : '#111',
                    borderLeft: `2px solid ${msg.isAI ? '#7C3AED' : '#1F1F1F'}`
                  }}>
                    <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: msg.isAI ? '#7C3AED' : '#444', letterSpacing: '0.1em', marginBottom: '6px' }}>
          ...
          {/* Reply */}
          <div style={{ border: '1px solid #1F1F1F', background: '#0D0D0D', padding: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
              ...
              <button onClick={handleSuggestReply} disabled={suggesting} style={{
                ...
                padding: '6px 10px', fontSize: '10px', fontFamily: 'JetBrains Mono, monospace',
