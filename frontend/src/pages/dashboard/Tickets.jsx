...
import { LayoutDashboard, Ticket, Users, LogOut, Plus, X, Menu } from 'lucide-react'
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.05em', cursor: 'pointer'
            }}
          >
            {showForm ? <><X size={12} /> CANCEL</> : <><Plus size={12} /> NEW</>}
          </button>
        </div>

        <div style={{ padding: '24px 20px', flex: 1 }}>
          <div style={{ marginBottom: '24px' }}>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', color: '#F5A623', letterSpacing: '0.1em', marginBottom: '6px' }}>SUPPORT</div>
            <h1 style={{ fontSize: '24px', fontWeight: 700, margin: 0, letterSpacing: '-0.02em' }}>Tickets</h1>
          </div>

          {/* Create Form */}
          {showForm && (
            <div style={{ border: '1px solid #1F1F1F', background: '#0D0D0D', padding: '24px', marginBottom: '24px' }}>
              <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '10px', color: '#F5A623', letterSpacing: '0.12em', marginBottom: '20px' }}>NEW TICKET</div>
              <form onSubmit={handleSubmit}>
                <div style={{ marginBottom: '16px' }}>
                  <label style={labelStyle}>TITLE</label>
                  <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} required placeholder="Brief description" style={inputStyle} />
                </div>
                <div style={{ marginBottom: '16px' }}>
                  <label style={labelStyle}>DESCRIPTION</label>
                  <textarea value={form.body} onChange={e => setForm({ ...form, body: e.target.value })} required placeholder="Detailed description..." rows={4} style={{ ...inputStyle, resize: 'vertical' }} />
                </div>
                ...
                <button type="submit" disabled={submitting} style={{
                  ...
                  border: 'none', padding: '10px 20px', fontSize: '11px', fontWeight: 700,
          ...
          <div style={{ border: '1px solid #1F1F1F' }}>
            {loading ? (
              ...
              <div style={{ padding: '48px', textAlign: 'center', color: '#444', fontFamily: 'JetBrains Mono, monospace', fontSize: '12px' }}>NO TICKETS YET</div>
            ) : (
                ...
                return (
                  <Link to={`/tickets/${ticket.id}`} key={ticket.id} style={{ textDecoration: 'none' }}>
                    <div style={{
                      ...
                      padding: '14px 16px',
                      borderBottom: i < tickets.length - 1 ? '1px solid #141414' : 'none',
                      borderLeft: `3px solid ${statusColor(ticket.status)}`,
                      cursor: 'pointer'
                    }}
